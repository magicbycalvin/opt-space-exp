function [v_c, omega_c] = turtlebot_TT(time,p,omega,Cp,Cpdot,time_info)

%% Initialize TT Variables
kp = 2;
kR = 0.5;


%% Determine pd, vd and Rt from Bernstein Poly
t0 = time_info(1); Tf = time_info(2); Ts = time_info(3);

pd = BernsteinPoly(Cp,time,t0,Tf);
pddot = BernsteinPoly(Cpdot,time,t0,Tf);
omegat = atan2(pddot(2),pddot(1));
vd = sqrt(pddot(1)^2+pddot(2)^2);
Rt = [cos(omegat) -sin(omegat); sin(omegat) cos(omegat)];
...
t_1 = Rt(:,1);
v_c = [(kp*ep+vd*t_1)'*t_1, 0]';

%% Set up desired frame and desired angular rate
b1D = (kp*ep+vd*t_1)/norm((kp*ep+vd*t_1));
b2D = [-b1D(2), b1D(1)]';
Rd = [b1D, b2D];
